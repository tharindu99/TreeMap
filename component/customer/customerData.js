...
const CustomerData = ({cusotmer,exception,journal,str}) => {
    ...
    const AmPmAnalysis = (data,AMPM,CRDR,MainTotal) => {
        ...
        if(dataM.length>0){
            ...
            return [
                ...
                {name:'Percentage('+((total*100)/MainTotal).toFixed(2)+'%)',label:{fontSize: 14,fontWeight: 'bold'}},
                {name:'AcctWithHighest'+AMPM+special_word, label:{fontSize: 14,fontWeight: 'bold'},
                    children:[
                        ...
                        {name:'Percentage('+((highest[1]*100)/MainTotal).toFixed(2)+'%)',label:{fontSize: 14,fontWeight: 'bold'}},
                    ]
                },
                {name:'AcctWithLowest'+AMPM+special_word, label:{fontSize: 14,fontWeight: 'bold'},
                    children:[
                        ...
                        {name:'Percentage('+((lowest[1]*100)/MainTotal).toFixed(2)+'%)',label:{fontSize: 14,fontWeight: 'bold'}},
                    ]
                },
    ...
    const TotalAnalyticsGenerator = (array) => {
        ...
        const exceptionForCutomer = exception.filter(d => d.IC4PROCUSTOMERID === cusotmerID)
        const RiskGroupBy = Array.from(d3.group(exceptionForCutomer, d => d.IC4PRORATING), ([name,children]) => ({name,children}))
        
        const StrForCutomer = str.filter(d => d.IC4PROCUSTOMERID === cusotmerID)
        ...
        const analytics_child = {
            ...
            children:[
                ...
                children: [
                    ...
                    children: AmPmAnalysis(array,'Am','Credit',(AMcredit+PMcredit)), label:{fontSize: 14,fontWeight: 'bold' }
                    },
                    {name:'PM('+NumberPattern(PMcredit)+','+pmCreditVoucher+')',label:{fontSize: 14,fontWeight: 'bold'},
                    children: AmPmAnalysis(array,'Pm','Credit',(AMcredit+PMcredit)), label:{fontSize: 14,fontWeight: 'bold' }
                    },
                ...
                children: [
                    ...
                    children: AmPmAnalysis(array,'Am','Debit',(AMdebit+PMdebit)), label:{fontSize: 14,fontWeight: 'bold' }
                    },
                    {name:'PM('+NumberPattern(PMdebit)+','+pmDebitVoucher+')',label:{fontSize: 14,fontWeight: 'bold'},
                    children: AmPmAnalysis(array,'Pm','Debit',(AMdebit+PMdebit)), label:{fontSize: 14,fontWeight: 'bold' }
                    },
